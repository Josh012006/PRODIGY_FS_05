...
export default function SettingsPage({user} : {user: User}) {
    ...
    return(
        <>
            {authorized && <div className="p-4">
                <div className="bg-cover bg-center bg-no-repeat rounded-full w-24 h-24 lg:w-36 lg:h-36 relative" style={{backgroundImage: `url("/profile_pictures/${user.profilePicture?? "unknown.jpg"}")`}} onClick={() => {setExpandProfPic(true)}}>
                    <span className="absolute right-0 bottom-0 z-10 rounded-full h-7 w-7 flex items-center justify-center bg-zinc-800" onClick={() => {setChangeProfPic(true)}}>
                        <i className="fa-solid fa-camera text-lg lg:text-xl"></i>
                    </span>
                </div>
                {expandProfPic && <div className="w-full h-full absolute top-0 left-0 z-10" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8))"}}>
                    <i className="fa-solid fa-x m-4 cursor-pointer" aria-hidden="true" onClick={() => {setExpandProfPic(false)}}></i>
                    <div className="h-full flex items-center">
                        <img className="w-full border-y border-gray-700" alt="media" src={`/posts/medias/${user.profilePicture}`} />
                    </div>
                </div>}
                {changeProfPic && <div className="w-full h-full absolute top-0 left-0 z-10" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8))"}}>
                    <i className="fa-solid fa-x m-4 cursor-pointer" aria-hidden="true" onClick={() => {setChangeProfPic(false)}}></i>
                    <form className="rounded-lg bg-neutral-700 w-11/12 sm:w-1/2 flex flex-col p-6 mx-auto" id="changeProfPicForm" onSubmit={handleChangeProfPic}>
                ...
                <div className="text-center p-2 lg:p-4 flex flex-col items-center gap-2">
                    ...
                    <p>{user.bio}</p>
                    <button type="button" className="mx-5 h-10 w-32 bg-zinc-800 rounded-md" onClick={() => {setChangeBio}}>Change the bio</button>
                </div>
                {changeBio && <div className="w-full h-full absolute top-0 left-0 z-10" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8))"}}>
                    <i className="fa-solid fa-x m-4 cursor-pointer" aria-hidden="true" onClick={() => {setChangeBio(false)}}></i>
                    <form className="rounded-lg bg-neutral-700 w-11/12 sm:w-1/2 flex flex-col p-6 mx-auto" id="changeBioForm" onSubmit={handleChangeBio}>
                ...
                <div className="flex flex-col gap-2 p-2 lg:p-4">
                    <h2 className="text-xl lg:text-2xl">Your story</h2>
                    ...
                    <div className="flex flex-col lg:flex-row items-center justify-around">
                        <button type="button" className="mx-5 h-10 min-w-32 bg-zinc-800 rounded-md" onClick={() => {setChangeStory}}>Change story</button>
                        <button type="button" className="mx-5 h-10 min-w-32 bg-zinc-800 rounded-md" onClick={() => {handleDeleteStory}}>No story!</button>
                    </div>
                    {changeStory && <div className="w-full h-full absolute top-0 left-0 z-10" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8))"}}>
                        <i className="fa-solid fa-x m-4 cursor-pointer" aria-hidden="true" onClick={() => {setChangeStory(false)}}></i>
                        <form className="rounded-lg bg-neutral-700 w-11/12 sm:w-1/2 flex flex-col p-6 mx-auto" id="changeStoryForm" onSubmit={handleChangeStory}>
                ...
                <div>
                    ...
                    <p className="my-2"><span className="font-bold">Email:</span> <span className="whitespace-normal">{user.email}</span></p>
                    <button className="mx-5 h-10 min-w-32 bg-zinc-800 rounded-md" type="button" onClick={() => {setChangePassword(true)}}>Change your password</button>
                </div>
                {changePassword && <div className="w-full h-full absolute top-0 left-0 z-10" style={{backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.8))"}}>
                    <i className="fa-solid fa-x m-4 cursor-pointer" aria-hidden="true" onClick={() => {setChangePassword(false)}}></i>
                    <form className="rounded-lg bg-neutral-700 w-11/12 sm:w-1/2 flex flex-col p-6 mx-auto" id="changePasswordForm" onSubmit={handleChangePassword}>
